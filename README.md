...
当前训练/测试脚本使用 embedding 级输入：
- `embeddings.npy`：`[N, D]`，默认 `D=768`
- `labels.npy`：`[N, C]`，如 `C=6`（`any, EDH, IPH, IVH, SAH, SDH`）
...
python script/train_meddinov3_head.py \
  ...
  --hidden-dim 256 \
  --dropout 0.2 \
  --output-dir outputs/head_train
```

输出：
- `outputs/head_train/best.pt`
- `outputs/head_train/last.pt`
- `outputs/head_train/train_summary.json`
...
python script/test_meddinov3_head.py \
  ...
  --checkpoint outputs/head_train/best.pt \
