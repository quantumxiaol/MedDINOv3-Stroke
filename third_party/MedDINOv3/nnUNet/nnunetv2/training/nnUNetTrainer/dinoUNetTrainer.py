...
def _load_env() -> None:
    ...
    try:
        ...
        return
    env_root = _find_env_root()
    if env_root is not None:
        load_dotenv(env_root / ".env", override=False)


def _get_env_path(env_key: str, required: bool = True) -> str | None:
    _load_env()
    value = os.getenv(env_key)
    if not value:
        if required:
            raise FileNotFoundError(
                f"{env_key} is not set. Please set it in .env (e.g. {env_key}=modelsweights/...)"
            )
        return None
...
class dinoUNetTrainer(nnUNetTrainer):
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        dinov3_pretrained_path = _get_env_path("DINOV3_CKPT_PATH", required=True)
...
class DinoUNet(nn.Module):
    ...
    def __init__(self,
                 ...
                 dinov3_pretrained_path: str | None = None,
        ...
        if not dinov3_pretrained_path:
            dinov3_pretrained_path = _get_env_path("DINOV3_CKPT_PATH", required=False)
